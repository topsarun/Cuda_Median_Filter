...
{
	int img_w = 1000; //IMG_Width
	int img_h = 1000; //IMG_Higth

	//Input File
	int dataLength = img_w*img_h;
	unsigned short *Input_Image_Host = new unsigned short[dataLength];//(unsigned short *)malloc(dataLength* sizeof(unsigned short));
	unsigned short *Output_Image_Host = new unsigned short[dataLength];//(unsigned short *)malloc(dataLength* sizeof(unsigned short));
	FILE *op1;
	...
	//TEST cudaMemcpy
	/*cudaMemcpy(Output_Image_Host, Input_Image, 2 * dataLength, cudaMemcpyDeviceToHost);
	for (int i = 0; i < dataLength; i++)
	{
		if (i%img_w == 0) printf("\n");
	...
	Cuda_debug << < grid, block >> >(Input_Image, Output_Image, img_w, img_h); //Cuda Median Filter
